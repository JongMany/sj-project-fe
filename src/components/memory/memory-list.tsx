...
function MemoryList({
  ...
  const editMemory = async (memoryId: string, changeValue: string)=> {
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setMemoryList(prev => prev.map((item) => item.id !== memoryId ? item : {
          ...item,
          description: changeValue,
        }));
      }
  ...
  return (
      <>
        <div className="overflow-scroll h-[80dvh] bg-gray-100 rounded-md text-black flex-1 px-4 py-4">
          {memoryList.map((memory) => <div key={memory.id} className="flex items-center justify-between mb-3">
            <div className="flex gap-x-2 items-center">
              <span className="w-[80px] font-semibold text-[16px]">{title[memory.type]}</span>
