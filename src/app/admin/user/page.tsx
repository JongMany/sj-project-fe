...
async function Page({searchParams: {id1 ,id2}}:Props) {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/v1/user/privacy/${encodedId}`);
      ...
      <div>
        ...
        <div>
          ...
          <div className='flex flex-col'>
            {data?.threads?.length > 0 ? data?.threads?.map((thread) => <div key={thread.threadId} className='flex gap-x-3'>
